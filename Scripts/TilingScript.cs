...
public class TilingScript : MonoBehaviour {
	...
	void Awake () {
		...
		// RESIZE PROPERLY
		int spriteColumns = (int) Mathf.Round (sprite.bounds.size.y);
		int spriteRows = (int) Mathf.Round (sprite.bounds.size.x);

		Vector3 tempScale = sprite.transform.localScale;
		tempScale.x = spriteRows;
		tempScale.y = spriteColumns;

		sprite.transform.localScale = tempScale;

		/*if(SpriteAlignment(gameObject) != SpriteAlignment.TopRight){
